...
class VLCPlayer:
    def __init__(self, root):
        ...
        # Preencher a caixa de texto com as URLs memorizadas
        self.populate_url_text()

        # Adicionar evento de duplo clique para selecionar uma URL e tocar
        self.url_text.bind("<Double-Button-1>", self.play_selected_url)
    ...
    # Função para preencher a caixa de texto com URLs guardadas
    def populate_url_text(self):
        for url in self.saved_urls:
            self.url_text.insert(tk.END, url + "\n")

    # Função para tocar a URL selecionada com duplo clique
    def play_selected_url(self, event):
        try:
            # Obter a linha onde o utilizador clicou duas vezes
            selected_line = self.url_text.get("insert linestart", "insert lineend").strip()
            if selected_line:
                self.media_path = selected_line
                self.play_media()
        except Exception as e:
            messagebox.showerror("Erro", f"Falha ao tocar a URL: {str(e)}")


# Função principal para iniciar o player
def main():
    root = tk.Tk()
    app = VLCPlayer(root)
    root.mainloop()
